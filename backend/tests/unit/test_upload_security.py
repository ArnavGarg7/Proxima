...
async def test_invalid_extension(security_service):
    file = UploadFile(filename="test.exe", file=io.BytesIO(b"MZ..."))
...
async def test_oversized_file(security_service):
    file = UploadFile(filename="big.txt", file=io.BytesIO(b"x" * 200))
    file.content_type = "text/plain"
...
async def test_magic_byte_mismatch(security_service):
    # Extension is pdf, but content is just "Not a PDF"
